...
MATCH (n {id: $source_memory_id})
RETURN COUNT(n) > 0 AS node_exists
"""

MEMORY_NORMALIZATION_QUERY = """
MATCH (e1:Memory)
WHERE e1.embedding IS NOT NULL AND e1.id = $source_memory_id

// Get semantic candidates
CALL db.index.vector.queryNodes('memory_embeddings', 20, e1.embedding)
...
MATCH (user:User)-[associated_with:ASSOCIATED_WITH]-(e)
WITH e, collect(associated_with) as all_associated_with
WHERE size(all_associated_with) > 1

// Keep the first mention relationship only
WITH e, all_mentions,
     all_associated_with[0] as primary_associated_with,
     all_associated_with[1..] as other_associated_with

// Mark primary associated_with as normalized
SET primary_associated_with.merged_at = datetime()

// Delete other mention relationships
FOREACH (rel IN other_associated_with | DELETE rel)

RETURN count(*) as associated_with_relationships_merged
"""
